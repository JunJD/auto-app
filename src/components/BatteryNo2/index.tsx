...
export default function BatteryNo() {
    ...
    const [isEndGarbled, setIsEndGarbled] = useState('2');
    const [startComplement, setStartComplement] = useState('000');
    const [endComplement, setEndComplement] = useState('AAA');
    ...
    const pauseWhileRef = useRef(false)
    const pause = () => {
        pauseWhileRef.current = true
    ...
    const handleEndPosition = (value: string) => {
        const num = +value
        // 非数字
        setEndComplement('');
        setBatteryNo(prev => {
            if (prev.length < num) return prev
            const reverseV = prev.split('').reverse()
            setEndPosition(value)
            if (num === 0) {
                // 如果有三个空格了，就删除最后一个空格
                const spaceL = reverseV.filter(it => it === ' ').length
                if (spaceL === 3) {
                    const findIndex = reverseV.findIndex(it => it === ' ')
                    reverseV.splice(findIndex, 1)
                }
                return reverseV.reverse().join('').trimEnd()
            }
            reverseV.splice(num, 0, ' ')
            return reverseV.reverse().join('').trimEnd()
        })
    }
    ...
    const handleEndComplement = (value: string) => {
        if (value.length - 1 < Number(endPosition)) {
            setEndComplement(value);
        }
    }
    ...
    const handleEndSelectChange = (e: any, newValue: string | null) => {
        if (!newValue) return
        setIsEndGarbled(newValue);
        setTimeout(() => {
            if (newValue === '2') {
                handleEndComplement(Array.from({ length: Number(endPosition) }).fill(0).map(it => 'A').join(''));
            } else if (newValue === "1") {
                handleEndComplement(Array.from({ length: Number(endPosition) }).fill(0).map(it => '0').join(''));
            }
        });
    }
    ...
    async function onSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        for (const _ of Array.from({ length: Number(exhaustiveQuantity) }).fill(0)) {
            ...
            if (current) {
                ...
                const start = item.slice(0, item.length - Number(endPosition))
                list.push(start)
            } else {
                break
            }
        ...
        for (const start of list) {
            resolveList.push(new Promise(async (resolve) => {
                ...
                do {
                    ...
                    if (current) {
                        currentString = current
                        const item = start + currentString
        ...
        <Stack spacing={2} height={'100%'}>
            ...
            >
                <Stack spacing={1}>
                    ...
                    <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                        <FormLabel>末尾位置(倒着数)</FormLabel>
                        <Input required name="endPosition" value={endPosition} onChange={(e) => handleEndPosition(e.target.value)} sx={{ flex: 1 }} />
                        <FormLabel>末尾补充码类型</FormLabel>
                        <Select name="isEndGarbled" value={isEndGarbled} onChange={handleEndSelectChange}>
                            <Option value="1">顺码</Option>
                            <Option value="2">乱码</Option>
                            <Option value="3">随机</Option>
                        </Select>
